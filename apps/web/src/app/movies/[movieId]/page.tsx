...
import { useFetchMovie, type DatesWithTheatreTimings, type Movie } from "./query";
...
export default function MoviePage() {
  ...
  const fetchMovieQuery = useFetchMovie(params.movieId);
  const [movie, setMovie] = useState<Movie | null>(null);
  const [datesWithTheatreTimings, setDatesWithTheatreTimings] =
    useState<DatesWithTheatreTimings | null>(null);
  ...
  if (fetchMovieQuery.error || !movie) {
    return (
  ...
  const activeDateKey =
    selectedDateKey && dateGroupKeys.includes(selectedDateKey) ? selectedDateKey : dateGroupKeys[0];

  const activeTheatres = activeDateKey ? datesWithTheatreTimings[activeDateKey] : undefined;
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <section className="relative overflow-hidden">
        ...
        {/* Decorative glow */}
        <div className="absolute left-1/2 top-0 h-125500px] -translate-x-1/2 rounded-full bg-red-600/20 blur-3xl" />
              ...
              <div>
                ...
                <div className="mb-8 flex items-center gap-3 overflow-x-auto pb-2">
                  ...
                  {dateGroupKeys.map((dateKey) => {
                    const firstSlotStart = Object.values(datesWithTheatreTimings[dateKey])[0]
                      ?.dates[0]?.start;
